...
yookassa = Blueprint('yookassa', __name__)

@yookassa.route('/yookassa-settings')
@login_required
def index():
    try:
        api_setting = ApiSettings.query.filter_by(is_active=True).first()
        if not api_setting:
            return render_template('yookassa-settings.html', settings=None)
        ...
        response = requests.get(f"{base_url}/yookassa/", headers=headers)

        if response.status_code == 200:
            settings = response.json()
            return render_template('yookassa-settings.html', settings=settings)
        else:
            return render_template('yookassa-settings.html', settings=None)
...
@yookassa.route('/yookassa-settings/add', methods=['POST'])
@login_required
def add_settings():
    try:
        data = request.json
...
@yookassa.route('/yookassa-settings/delete', methods=['POST'])
@login_required
def delete_settings():
    try:
        data = request.json
        ...
        print(f"Отправка DELETE запроса на {base_url}/yookassa/ с shop_id: {shop_id}")
        ...
        print(f"Ошибка при удалении настроек YooKassa: {error_details}")
        return jsonify({"success": False, "message": f"Ошибка: {str(e)}"})

@yookassa.route('/yookassa-settings/get', methods=['GET'])
@login_required
def get_settings():
    try:
        api_setting = ApiSettings.query.filter_by(is_active=True).first()
