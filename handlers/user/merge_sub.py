...
async def process_continue_merge(callback: CallbackQuery):
    ...
    try:
        ...
        happ_url = f"https://auto.syslab.space/happ?vless={url_encoded_subs}"
        v2raytun_url = f"https://auto.syslab.space/v2raytun?vless={url_encoded_subs}"
        streisand_url = f"https://auto.syslab.space/streisand?vless={url_encoded_subs}"


        keyboard = get_back_keyboard()
        keyboard.inline_keyboard.insert(0, [
            InlineKeyboardButton(text="Streisand", url=streisand_url),
            InlineKeyboardButton(text="Happ Plus", url=happ_url),
        ...
        message_text = (
            ...
            "</blockquote>"
            "💡 Выберите приложение для автоматической настройки или скопируйте ключ вручную. Предпочтительно используйте Streisand."
        )
